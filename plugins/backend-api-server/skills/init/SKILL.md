---
description: 프로젝트에 backend-api-server 플러그인 document/ 구조와 아키텍처 템플릿을 초기화한다
---
...
이 스킬의 `templates/document/` 하위 파일을 프로젝트 루트의 `document/`로 복사한다.
...
존재하지 않는 디렉토리는 자동 생성:
```bash
mkdir -p document/{architecture,exec-plans,api-spec,product-specs}
```
